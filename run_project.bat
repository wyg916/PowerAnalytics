...
if not exist "%PYTHON_EXE%" (
    echo [ERROR] Approved project Python runtime was not found.
...
if not defined LOCAL_RUNTIME_CONFIG (
    ...
    echo [TIP] Set LOCAL_RUNTIME_CONFIG to a Git-ignored local env file.
    if "%RC_NO_PAUSE%"=="0" pause
    goto end_error
)
...
if not defined LOCAL_DATABASE_CONFIG (
    ...
    echo [TIP] Set LOCAL_DATABASE_CONFIG to the approved local database identity file.
    if "%RC_NO_PAUSE%"=="0" pause
    goto end_error
)
...
if not defined QDRANT_RUNTIME_CONFIG (
    ...
    echo [TIP] Set QDRANT_RUNTIME_CONFIG to the approved local RAG-R1 runtime.env.
    if "%RC_NO_PAUSE%"=="0" pause
    goto end_error
)
...
if not defined RAG_MODEL_CONFIG (
    ...
    echo [TIP] Set RAG_MODEL_CONFIG to the approved local model-profile.env.
    if "%RC_NO_PAUSE%"=="0" pause
    goto end_error
)
...
docker compose --env-file "%QDRANT_RUNTIME_CONFIG%" -f "%~dp0deploy\rag-r1\docker-compose.qdrant.yml" --profile rag-r1-qdrant up -d qdrant 2>&1
if errorlevel 1 goto rc_failed
...
echo [INFO] Backend health: http://127.0.0.1:%WEB_BACKEND_PORT%/api/health
