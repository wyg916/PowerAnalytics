...
def _approved_qdrant_root(env_file: Path) -> Path:
    raw = os.environ.get("RAG_RELEASE_ALLOWED_QDRANT_ROOT", "").strip()
    candidate = Path(raw) if raw else Path()
    if not raw or not candidate.is_absolute():
        raise EnterpriseKnowledgeUnavailable("qdrant_admin_root_missing")
    try:
        root = candidate.resolve(strict=True)
        resolved_env = env_file.resolve(strict=True)
    except OSError as exc:
        raise EnterpriseKnowledgeUnavailable("qdrant_admin_profile_unavailable") from exc
    if (
        root.parent == root
        or tuple(part.casefold() for part in root.parts[-2:]) != QDRANT_ROOT_SUFFIX
        or resolved_env != root / "secrets" / "runtime.env"
    ):
        raise EnterpriseKnowledgeUnavailable("qdrant_admin_root_invalid")
    try:
...
class QdrantReleaseAdmin:
    def __init__(self, env_file: Path):
        expected_root = _approved_qdrant_root(env_file)
        values = dotenv_values(env_file)
        raw_root = str(values.get("RAG_R1_QDRANT_ROOT") or "").strip()
        root_candidate = Path(raw_root) if raw_root else Path()
        try:
            root = root_candidate.resolve(strict=True)
        except OSError as exc:
            raise EnterpriseKnowledgeUnavailable("qdrant_admin_profile_invalid") from exc
        ...
        if not raw_root or not root_candidate.is_absolute() or root != expected_root or not admin or admin == reader:
            raise EnterpriseKnowledgeUnavailable("qdrant_admin_profile_invalid")
        expected_ca_path = root / "tls" / "ca-cert.pem"
        try:
            ca_path = expected_ca_path.resolve(strict=True)
        except OSError as exc:
            raise EnterpriseKnowledgeUnavailable("qdrant_admin_ca_unavailable") from exc
