...
from backend.app.services import rag_qdrant_transport
...
def test_frozen_rag_r1_candidate_evidence_is_accepted_without_alias_switch(
    monkeypatch, tmp_path: Path
) -> None:
    payloads = {
        "candidate_build_report.json": {"status": "PASS", "release_id": "RAG-R1"},
        "candidate_collection_report.json": {
            "status": "PASS",
            "release_id": "RAG-R1",
        ...
        "candidate_release_envelope.json": {
    ...
    monkeypatch.setattr(
        rag_qdrant_transport, "EXPECTED_RELEASE_ROOT", tmp_path.resolve()
    )
