...
def test_model_center_page_is_lifecycle_workbench():
    ...
    assert "治理记录" in text
    assert "disabled={!canRunTraining || !rollbackVersion}" in text
    assert "disabled={!canRunTraining || !data.rollback.options.length}" in text
    assert "训练任务已进入 Worker 队列" in text
