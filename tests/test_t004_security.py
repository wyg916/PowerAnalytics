...
def _production_env(monkeypatch, *, secret: str | None = STRONG_TEST_SECRET, auth: str | None = "1", admin: str | None = "1") -> None:
    ...
    monkeypatch.setenv(
        "DATABASE_URL",
        "postgresql+psycopg://t004_app:unit-test-only@127.0.0.1:1/postgres",
    )
    monkeypatch.setenv(
        "SECURITY_DATABASE_URL",
        "postgresql+psycopg://t004_security:unit-test-only@127.0.0.1:1/postgres",
    )
...
def test_day2_preflight_static_gates_pass():
    ...
    assert migration_result["files_scanned"] == 24
    assert any(
