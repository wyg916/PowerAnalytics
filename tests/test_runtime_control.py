...
def test_runtime_files_accept_explicit_cross_machine_paths(monkeypatch, tmp_path) -> None:
    profile = tmp_path / "profile.env"
    database = tmp_path / "database.env"
    runtime = tmp_path / "runtime.env"
    monkeypatch.setenv("RAG_PREPRODUCTION_CONFIG", str(profile))
    monkeypatch.setenv("LOCAL_DATABASE_CONFIG", str(database))
    monkeypatch.setenv("LOCAL_RUNTIME_CONFIG", str(runtime))

    assert runtime_control._runtime_files() == [
        profile.resolve(),
        database.resolve(),
        runtime.resolve(),
    ]
