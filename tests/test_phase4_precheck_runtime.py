...
def test_default_knowledge_worker_covers_landed_non_forecast_business_queues(monkeypatch):
    monkeypatch.setenv("PHASE4_WORKER_ROLE", "knowledge")
    monkeypatch.delenv("KNOWLEDGE_CELERY_QUEUES", raising=False)

    command = runtime._celery_worker_command()

    queue_argument = next(item for item in command if item.startswith("--queues="))
    queues = set(queue_argument.removeprefix("--queues=").split(","))
    assert {"rag", "embedding", "report", "data_sync", "report_daily", "default"} <= queues
