...
def test_profile_freezes_versions_paths_dimensions_and_fallbacks(tmp_path):
    ...
    assert f"RAG_EMBEDDING_MODEL_PATH={embedding.as_posix()}" in content
    assert f"RAG_RERANK_MODEL_PATH={reranker.as_posix()}" in content
    assert f"RAG_QDRANT_TLS_CA_PATH={(qdrant / 'tls' / 'ca-cert.pem').as_posix()}" in content


def test_profile_rejects_runtime_version_or_fallback_drift(tmp_path):
    admission, smoke = _reports()
    paths = {
        "embedding_root": tmp_path / "bge-large-zh-v1.5",
        "reranker_root": tmp_path / "bge-reranker-v2-m3",
        "qdrant_root": tmp_path / "rag-r1" / "qdrant",
    }
    ...
    with pytest.raises(ModelProfileError, match="runtime_mismatch"):
        render_profile(admission, drifted, **paths)
    ...
    with pytest.raises(ModelProfileError, match="runtime_unsafe"):
        render_profile(admission, fallback, **paths)
