...
def test_permission_matrix_exactly_covers_runtime_routes_and_is_reproducible():
    ...
    assert len(rows) == 208
    assert len({row["Path"] for row in rows}) == 192
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("viewer", "POST", "/api/models/center/training/start", 403),
        ("analyst", "POST", "/api/models/center/training/start", 403),
        ("admin", "POST", "/api/models/center/training/start", 200),
        ("viewer", "POST", "/api/models/center/rollback", 403),
        ("analyst", "POST", "/api/models/center/rollback", 403),
        ("admin", "POST", "/api/models/center/rollback", 200),
