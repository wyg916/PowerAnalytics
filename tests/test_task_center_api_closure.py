...
def test_task_start_returns_503_instead_of_fake_postgres_pending(monkeypatch):
    ...
    assert response.json()["detail"] == "celery unavailable"
...
def test_task_start_dispatches_business_task_types(monkeypatch):
    ...
    for kind in ["price_predict", "data_sync", "report_daily", "health_check"]:
...
def test_task_retry_returns_503_instead_of_fake_postgres_pending(monkeypatch):
    monkeypatch.setattr(
        task_endpoint,
        "get_task_record",
