...
def test_sc006_uses_one_shared_three_page_header_and_layout_system() -> None:
    ...
    assert len(re.findall(r"<PageHeader(?:\s|>)", page)) == 1
    assert page.count("<StrategyCenterPageHeader") == 1
    assert page.count("<StrategyMetricStrip") == 1
    assert "tabs={strategyTabs.filter(" in page
    assert "items={tabs}" in page
    assert "activeKey={activeTabKey}" in page
    assert "onChange={onTabChange}" in page
    assert "onTabChange={handleStrategyTabChange}" in page
