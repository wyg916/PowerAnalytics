...
def test_activation_failure_rolls_back_entire_transaction(isolated_engine: sa.Engine):
    ...
    try:
        with pytest.raises(ModelFactError, match="事务已回滚"):
...
def test_governed_rollback_commits_state_and_event_atomically(isolated_engine: sa.Engine):
    service = ModelFactService(isolated_engine)
    for version in ("rollback-stable", "rollback-current"):
        service.register_candidate(_candidate(version))
        service.validate_candidate(version, "price", "da_price")
    service.activate_model("rollback-stable", "price", "da_price")
    service.activate_model("rollback-current", "price", "da_price")

    result = service.rollback_active_model(
        "rollback-stable",
        "price",
        "da_price",
        governance_event={
            "event_id": "event-rollback-success",
            "action": "model.rollback",
            "operator": "admin",
            "reason": "controlled rollback acceptance",
            "metadata": {"scope": "isolated"},
        },
    )

    assert result["model_version"] == "rollback-stable"
    assert result["previous_active_version"] == "rollback-current"
    with isolated_engine.connect() as conn:
        event_row = conn.execute(
            text(
                "SELECT action, target_version, source_version, operator, reason "
                "FROM model_governance_events WHERE event_id = 'event-rollback-success'"
            )
        ).mappings().one()
    assert dict(event_row) == {
        "action": "model.rollback",
        "target_version": "rollback-stable",
        "source_version": "rollback-current",
        "operator": "admin",
        "reason": "controlled rollback acceptance",
    }


def test_governance_event_failure_rolls_back_model_switch(isolated_engine: sa.Engine):
    service = ModelFactService(isolated_engine)
    for version in ("event-stable", "event-current"):
        service.register_candidate(_candidate(version))
        service.validate_candidate(version, "price", "da_price")
    service.activate_model("event-stable", "price", "da_price")
    service.activate_model("event-current", "price", "da_price")

    def fail_event(_conn, _cursor, statement, _params, _context, _many):
        if "insert into model_governance_events" in " ".join(statement.lower().split()):
            raise RuntimeError("injected governance failure")

    event.listen(isolated_engine, "before_cursor_execute", fail_event)
    try:
        with pytest.raises(ModelFactError, match="事务已回滚"):
            service.rollback_active_model(
                "event-stable",
                "price",
                "da_price",
                governance_event={
                    "event_id": "event-rollback-failed",
                    "action": "model.rollback",
                    "operator": "admin",
        ...
        event.remove(isolated_engine, "before_cursor_execute", fail_event)

    assert service.get_active_model("price", "da_price")["model_version"] == "event-current"
    assert service.get_model("event-stable", "price", "da_price")["status"] == "archived"
    with isolated_engine.connect() as conn:
        assert conn.execute(
            text("SELECT COUNT(*) FROM model_governance_events WHERE event_id = 'event-rollback-failed'")
        ).scalar_one() == 0
