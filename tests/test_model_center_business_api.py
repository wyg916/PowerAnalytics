...
def test_model_center_training_start_endpoint(monkeypatch):
    monkeypatch.setattr(model_endpoint, "start_model_training", lambda user, payload: {"task_id": "train_x", "status": "queued"})
...
def test_model_center_training_start_returns_503_without_worker(monkeypatch):
    monkeypatch.setattr(
        model_endpoint,
        "start_model_training",
        lambda user, payload: (_ for _ in ()).throw(RuntimeError("forecast worker unavailable")),
    )
    response = client.post(
        "/api/models/center/training/start",
        json={"model_type": "负荷预测模型", "region": "浙江省"},
    )
    assert response.status_code == 503
    assert response.json()["detail"] == "forecast worker unavailable"
...
def test_model_center_activate_failure_is_not_http_200(monkeypatch):
    monkeypatch.setattr(
        model_endpoint,
        "activate_model_version",
        lambda *args, **kwargs: {"success": False, "message": "candidate not validated"},
    )
    monkeypatch.setattr(model_endpoint, "write_audit_log", lambda **_: True)
    response = client.post(
        "/api/models/center/activate",
        json={"version": "candidate-invalid", "reason": "acceptance"},
    )
    assert response.status_code == 409
    assert response.json()["detail"]["success"] is False


def test_model_center_rollback_endpoint(monkeypatch):
    monkeypatch.setattr(
        model_endpoint,
        "rollback_model_version",
        lambda version, user, reason, **_: {
            "success": True,
            "active_version": version,
            "previous_active_version": "v3.3.0",
        },
    )
    monkeypatch.setattr(model_endpoint, "write_audit_log", lambda **_: True)
    response = client.post(
        "/api/models/center/rollback",
        json={"version": "v3.2.1", "reason": "rollback acceptance"},
    )
    assert response.status_code == 200
    assert response.json()["active_version"] == "v3.2.1"
