...
def _required_test_password() -> str:
    password = os.environ.get("FINAL_FUNCTIONAL_TEST_PASSWORD", "").strip()
    if not password:
        raise RuntimeError("FINAL_FUNCTIONAL_TEST_PASSWORD must be set by the caller")
    return password
...
def login(base_url: str, username: str) -> tuple[str, dict[str, Any]]:
    result = request_json(
        ...
        "POST",
        "/api/auth/login",
        payload={"username": username, "password": _required_test_password()},
    )
