...
RUNTIME_ROOT_SUFFIX = ("rag-r1", "qdrant")
...
def _external_runtime_root(root: Path) -> Path:
    if not root.is_absolute():
        raise QdrantBootstrapError("qdrant_runtime_root_must_be_absolute")
    resolved = root.resolve()
    if resolved.parent == resolved:
        raise QdrantBootstrapError("qdrant_runtime_root_rejected")
    if tuple(part.casefold() for part in resolved.parts[-2:]) != RUNTIME_ROOT_SUFFIX:
        raise QdrantBootstrapError("qdrant_runtime_root_layout_invalid")
    try:
...
def validate_root(root: Path) -> Path:
    resolved = _external_runtime_root(root)
    if resolved.exists():
...
def resume_bootstrap(root: Path, digest: str) -> dict[str, Any]:
    root = _external_runtime_root(root)
    if not root.is_dir():
