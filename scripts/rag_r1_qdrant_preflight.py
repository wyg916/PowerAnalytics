...
def _runtime_root(values: Mapping[str, str]) -> Path:
    ...
    if not candidate.is_absolute():
        raise QdrantPreflightError("qdrant_runtime_root_must_be_absolute")
    root = candidate.resolve()
    if root.parent == root:
        raise QdrantPreflightError("qdrant_runtime_root_rejected")
    if tuple(part.casefold() for part in root.parts[-2:]) != RUNTIME_ROOT_SUFFIX:
        raise QdrantPreflightError("qdrant_runtime_root_layout_invalid")
    try:
...
def inspect_preflight(
    ...
    result = {
        ...
        "runtime_root_anchor": root.anchor,
        "assets": asset_status,
