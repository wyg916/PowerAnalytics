...
def _resolve_qdrant_runtime(qdrant_env: Path) -> tuple[Path, Path]:
    if not qdrant_env.is_absolute():
        raise RuntimeError("Release worker Qdrant config must be an absolute path")
    try:
        qdrant_path = qdrant_env.resolve(strict=True)
    except OSError as exc:
        raise RuntimeError("Release worker Qdrant config does not exist") from exc
    root = qdrant_path.parent.parent
    if (
        qdrant_path != root / "secrets" / "runtime.env"
        or root.parent == root
        or tuple(part.casefold() for part in root.parts[-2:]) != ("rag-r1", "qdrant")
    ):
        raise RuntimeError("Release worker Qdrant config has an invalid runtime layout")
    try:
        root.relative_to(PROJECT_ROOT)
    except ValueError:
        pass
    else:
        raise RuntimeError("Release worker Qdrant config must be outside the repository")
    values = _read_env_values(qdrant_path)
    declared_raw = values.get("RAG_R1_QDRANT_ROOT", "").strip()
    declared = Path(declared_raw) if declared_raw else Path()
    if not declared_raw or not declared.is_absolute() or declared.resolve() != root:
        raise RuntimeError("Release worker Qdrant root does not match its runtime config")
    return qdrant_path, root
...
def configure_runtime(runtime_configs: Sequence[Path], qdrant_env: Path) -> None:
    ...
    qdrant_path, qdrant_root = _resolve_qdrant_runtime(qdrant_env)
    os.environ["RAG_RELEASE_DATABASE_URL"] = database_url
    os.environ["RAG_RELEASE_ALLOWED_QDRANT_ROOT"] = str(qdrant_root)
